...
const connectSocket = (token: string) => {
  ...
  newSocket.on('message_read_update', (data) => {
    ...
    // 해당 메시지의 읽지 않은 사용자 수를 다시 조회하여 업데이트
  ...
  newSocket.on('messages_read', (data) => {
    console.log('메시지들이 읽힘:', data);
    // 현재 채팅방의 메시지 목록 새로고침
    if (selectedChatRoom === data.roomId) {
      loadMessages(data.roomId);
    }
    // 채팅방 목록도 업데이트 (읽지 않은 메시지 수 변경)
    loadChatRooms();
  });
  ...
  newSocket.on('receive_message', (message) => {
    ...
    // 다른 사용자의 메시지인 경우 자동으로 읽음 처리
    // (현재 해당 채팅방을 보고 있을 때만)
    if (message.sender_id !== user?.id && selectedChatRoom === message.room_id) {
